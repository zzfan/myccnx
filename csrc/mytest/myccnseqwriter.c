/*************************************************************************
	> File Name: myccnqwriter.c
	> Author: zzfan
	> Mail: zzfan@mail.ustc.edu.cn 
	> Created Time: Tue 01 Apr 2014 02:42:17 PM HKT
 ************************************************************************/
...
//    fgets(buff,128,fd);
    ...
    fgets(buff,128,fd);
    scanf (buff,  "%s %lu %d %d %d %d %d %d %d %lu %d %d %d %d %d %d %d", t->eth,&t->eth_in,&a,&b,&c,&d,&e,&f,&g,&t->eth_out,&h,&i,&j,&k,&l,&m,&n);
    fclose(fd);
}
...
int main(int argc, char **argv)
{
   ...
   // argc -= optind;
   // argv += optind;
    char *statusUrl = "ccnx:/server4"; //repository输出路径
   ...
   // fprintf(stderr,"%s",argv[0]);
    ...
    CPU_OCCUPY cpu_stat1;  //声明存储cpu占用率、内存占用率、网络连接占用率变量
